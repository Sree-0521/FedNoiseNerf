...
class SatelliteDataset(Dataset):
    def __init__(self, root_dir, img_dir, split="train", img_downscale=1.0, cache_dir=None, noise_level=0.0):
        """
        ...
        self.noise_level = noise_level if self.train else 0.0
    ...
    def load_data(self, json_files, verbose=False):
        ...
        for t, json_p in enumerate(json_files):
            ...
            if self.noise_level > 0.0:
                rgbs = torch.clamp(rgbs + self.noise_level * torch.randn_like(rgbs), 0.0, 1.0)
